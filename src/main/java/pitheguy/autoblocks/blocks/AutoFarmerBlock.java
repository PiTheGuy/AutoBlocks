...
public class AutoFarmerBlock extends BaseEntityBlock {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(Level level, BlockState state, BlockEntityType<T> blockEntityType) {
        return level.isClientSide ? null : BaseEntityBlock.createTickerHelper(blockEntityType, ModBlocKEntityTypes.AUTO_FARMER.get(), ((level1, pos, state1, blockEntity) -> blockEntity.tick()));
    }
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (level != null && !level.isClientSide()) {
            ...
            if (tile instanceof AutoFarmerBlockEntity farmer) {
                NetworkHooks.openScreen((ServerPlayer) player, farmer, pos);
    ...
    public void onRemove(BlockState state, Level level, BlockPos pos, BlockState newState, boolean isMoving) {
        BlockEntity tile = level.getBlockEntity(pos);
        if (tile instanceof AutoFarmerBlockEntity farmer) farmer.discard(level, pos);
