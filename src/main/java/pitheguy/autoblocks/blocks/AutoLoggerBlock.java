...
public class AutoLoggerBlock extends BaseEntityBlock {
    ...
    public BlockEntity newBlockEntity(BlockPos pos, BlockState state) {
        return new AutoLoggerBlockEntity(pos, state);
    }
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (level != null && !level.isClientSide()) {
            ...
            if (tile instanceof AutoLoggerBlockEntity logger) {
    ...
    public void onRemove(BlockState state, Level level, BlockPos pos, BlockState newState, boolean isMoving) {
        BlockEntity tile = level.getBlockEntity(pos);
        if (tile instanceof AutoLoggerBlockEntity logger) logger.discard(level, pos);
